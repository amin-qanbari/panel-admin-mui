...
const MoviesList = () => {
  ...
  const transform = () => {
    return movies.map(item => {
      return [
        <img src={item.image.url} alt="img" height={50} width={50} onClick={()=>onItemClick(item)}/>,
        item.title,
        item.audioLanguage,
      ...
      <Modal open={openModal} onClose={()=>setOpenModal(false)}>
        <div className={classes.container}>
          <Paper className={classes.paper}>
            ...
            <img src={selectedMovie.image && selectedMovie.image.url} alt="img" width={300} height={200}/>
